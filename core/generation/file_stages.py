...
def handle_kr_file_context_status(
    *,
    ...
    if status == "textgrid_missing":
        log_fn(f"[WARN] {fname}: TextGrid를 찾지 못해 해당 파일은 원본 행을 유지합니다.")
    ...
    if status == "textgrid_load_failed":
        ...
        log_fn(f"[WARN] {fname}: TextGrid 로드 실패로 원본 행을 유지합니다. ({err_msg})")
    ...
    if status == "tier_missing":
        log_fn(f"[WARN] {fname}: phones tier가 없어 원본 행을 유지합니다.")
...
def handle_kr_loop_prep_status(
    *,
    ...
    if status == "empty_intervals":
        log_fn(f"[WARN] {fname}: 유효한 음소 구간이 없어 원본 행을 유지합니다.")
    ...
    if status == "no_valid_alias":
        log_fn(f"[WARN] {fname}: 유효한 alias를 해석하지 못해 원본 행을 유지합니다.")
