...
# Phonetically meaningful effect suffixes that must NOT be stripped even when
# the prefix type matches: "ga_fry01" → "ga_fry" (strip "01"), keep "fry".
_PROTECTED_SUFFIX_WORDS: frozenset[str] = frozenset({"fry", "vf"})
...
def _try_strip_trailing_separator_suffix(
    ...
    for sep in ("_", " "):
        ...
        if not prefix or not suffix:
            continue
        if suffix.lower() in _PROTECTED_SUFFIX_WORDS:
            continue
