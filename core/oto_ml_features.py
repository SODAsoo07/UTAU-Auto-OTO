...
def _classify_kr_breath_alias_kind(alias: str, alias_type: str = "") -> str:
    ...
    def _is_tail_marker(tok: str) -> bool:
        ...
        if low in {
            ...
            return True
        return any(h in low for h in ("breath", "inhale", "exhale", "asp", "air", "숨", "흡", "호", "息", "吸", "吐"))

    if len(parts) >= 2:
        left = parts[0].strip().lower()
        right = parts[-1].strip()
        left_is_vowel_like = left in _GENERIC_VOWELS or left in {"a", "e", "i", "o", "u"} or bool(re.fullmatch(r"[\u3041-\u3096\u30A1-\u30FA\u30FC]+", parts[0].strip()))
        if left_is_vowel_like and _is_tail_marker(right):
            return "tail_breath"
