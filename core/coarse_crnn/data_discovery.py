...
from core.coarse_crnn.lab_io import find_alignment_label, label_compatibility_ratio, load_aligned_segments
...
def _public_search_roots(base: Path, *, scan_all: bool) -> list[Path]:
    ...
    for child in base.iterdir():
        ...
        if lowered in {"csd", "tiger_ja", "ofutonp", "ofuton_p_utagoe_db", "oniku_kurumi_utagoe_db"}:
            roots.append(child)
            continue
        if any(token in lowered for token in ("utagoe", "kiritan", "nit070", "波音", "歌声")):
            roots.append(child)
            continue
    return roots
