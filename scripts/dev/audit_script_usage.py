...
SCRIPT_SCAN_PREFIXES = ("scripts/", "tests/scripts/")
...
def _list_script_files(tracked_files: List[str]) -> List[str]:
    ...
    for rel in tracked_files:
        ...
        if not any(rel_norm.startswith(prefix) for prefix in SCRIPT_SCAN_PREFIXES):
            continue
...
def _classify_refs(script_rel: str, refs: List[str]) -> Tuple[List[str], List[str]]:
    ...
    for ref in refs:
        ...
        if ref_norm == script_rel:
            continue
        if any(ref_norm.startswith(prefix) for prefix in SCRIPT_SCAN_PREFIXES):
