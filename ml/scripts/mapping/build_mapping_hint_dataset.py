...
from core.mapping_supervised_training import build_mapping_hint_dataset
from core.runtime_encoding import bootstrap_utf8_runtime


def main() -> int:
    bootstrap_utf8_runtime()
    ap = argparse.ArgumentParser(description="Build mapping-hint candidate dataset CSV.")
    ...
    ap.add_argument("--source-csv", default="", help="Optional prebuilt mapping-supervised CSV")
    ap.add_argument(
    ...
    ap.add_argument(
        "--blank-attach-threshold",
        type=float,
        default=0.58,
        help="Threshold for blank_attach_label from blank_conf",
    )
    ap.add_argument(
        "--auto-oto-policy",
    ...
    stats = build_mapping_hint_dataset(
        ...
        blank_attach_threshold=float(args.blank_attach_threshold),
    )
