...
class RuntimeLoadingTests(unittest.TestCase):
    ...
    def test_load_coupled_backend_returns_none(self):
        with tempfile.TemporaryDirectory() as td:
            with open(os.path.join(td, "model_meta.json"), "w", encoding="utf-8") as f:
    ...
    def test_predict_oto_deltas_rejects_coupled_backend(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Unsupported OTO ML backend"):
            predict_oto_deltas(bundle, {"language": "korean"})
